...
Stream Deck plugins run continuously in the background. Elgato's guidelines require
low idle CPU and minimal memory footprint. Use the tools below to measure and reduce both.

### Always-on: memory logging

`profiling::log_memory(label)` reads the process RSS from the OS and emits a structured
`INFO` log line. Zero overhead beyond a single OS syscall. Call it at key moments:

```rust
profiling::log_memory("startup");          // already in main.rs
profiling::log_memory("after-data-load");  // add wherever useful
profiling::log_memory("cache-evicted");    // track effect of cleanup
```

Search the log file for `"memory"` to extract all readings:

```powershell
Get-Content "$env:APPDATA\Elgato\StreamDeck\Plugins\icu.veelume.starcitizen.sdPlugin\plugin.log" |
    Select-String '"memory"'
```

### Heap profiling (dhat)

Finds what is allocating memory and why it is retained.

```bash
# Build — full optimisations + debug symbols so call stacks are readable
cargo build --features heap-profile --profile release-debug

# Copy exe to bundle
Copy-Item target/release-debug/plugin.exe `
    icu.veelume.starcitizen.sdPlugin/bin/icu.veelume.starcitizen.exe

# Restart the plugin and exercise the feature you want to profile
streamdeck restart icu.veelume.starcitizen

# Stop the plugin — this flushes dhat-heap.json to the current directory
...
**What to look for:**
- **Total heap**: peak bytes allocated — if this grows after each keypress, you have a leak
- **Max live**: bytes alive at peak — this is your steady-state memory budget
- **Retained trees**: allocation call stacks that are live at peak — start here

### CPU / async profiling (tokio-console)

Shows per-task CPU time, poll durations, and waker activity in real time. Best for
finding tasks that spin too often or block the async runtime.

```bash
# One-time install
cargo install tokio-console

# Build (requires unstable tokio tracing)
$env:RUSTFLAGS = "--cfg tokio_unstable"
cargo build --features tokio-console

# Copy exe and restart, then in a separate terminal:
tokio-console
```

> **Note:** the `tokio-console` feature replaces the normal file-based tracing
> subscriber, so the plugin's log file will be empty during a console session.

### CPU profiling with samply (Windows)

Sampling profiler that records real CPU call stacks and opens them in the Firefox
Profiler UI. Good for finding hot loops in synchronous code.
...
# Profile (fill in the actual args from the Stream Deck launch command)
samply record -- .\icu.veelume.starcitizen.sdPlugin\bin\icu.veelume.starcitizen.exe `
    -port 28196 -pluginUUID <uuid> -registerEvent <event> -info <info>
